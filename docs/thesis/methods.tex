...
canonical core algorithm. It is the de-facto reference in subsequent
...
Because full-grid vectorization may exceed memory, the
candidate evaluates chunks of $K$ grid points at a time and
sweeps $K$ for breadth.
...
the same work or interfere. With no documented canonical
composition of \texttt{torch.compile} with \texttt{vmap} on
the loss-grid pattern, the composition verdict is empirical.
...
for parallel grid-point evaluation. The scheduling mechanism
is dynamic self-scheduling~\cite{polychronopoulos1987gss}:
idle workers poll a shared queue and claim one task at a time.
Richer scheduler-family policies (HEFT-style cost-model
scheduling, priority-based work-stealing in
StarPU~\cite{augonnet2011starpu}) are not candidates because
loss-grid's per-task work is uniform. The predictor variable
is the CPU/GPU throughput
ratio~\cite{gallet2021heterogeneous}: near or above parity,
the CPU can absorb a non-trivial share of tasks; far below
parity, the GPU-only baseline is already near-optimal.
...
To study ratios beyond the native operating point, the method
exposes an optional \texttt{gpu\_slowdown\_factor} that
injects a controlled post-kernel delay on the GPU worker. The
delay moves the operating point along the ratio axis, allowing
the ratio-versus-speedup relationship to be measured across
...
matches RQ1's key, making the composition well-defined and
the amortization claim keyed on the same identity.
...
selected tuple is assumed stable across checkpoints, allowing
the calibration cost paid by the first checkpoint to be reused
for the rest. This adapts the autotune-and-cache pattern of
ATLAS~\cite{whaley1998atlas} from a BLAS-operation key to a
workload-level key. The cache key is (machine, workload =
architecture + dataset + loss + grid spec); the cached value
is the hetero-config tuple selection. Cache reuse is invalid
if the selected mode is not stable across the session.
...
CPU/GPU split); the gate is a relative-equivalence criterion
rather than bitwise equality. Three parts:
\begin{enumerate}
...
default~\cite{pytorch_allclose}; with $\text{atol} = 0$,
