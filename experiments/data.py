...
class _Cifar10Dataset(Dataset):
    def __init__(self, root: Path, subset_size: int):
        cifar_batches = (
            root if root.name == CIFAR10_DIRNAME and root.exists() else root / CIFAR10_DIRNAME
        )
        ...
        self.limit = (
            min(int(subset_size), len(self.dataset)) if subset_size > 0 else len(self.dataset)
        )
...
class _MnistDataset(Dataset):
    def __init__(self, root: Path, subset_size: int):
        mnist_root = root.parent if root.name == MNIST_DIRNAME and root.exists() else root
...
def build_dataset(task: MLTaskSpec, seed: int) -> Dataset:
    dataset_name = task.dataset.name
    root = resolve_asset(task.dataset.path)
    if dataset_name == "cifar10":
        return _Cifar10Dataset(root, task.dataset.sample_count)
    if dataset_name == "mnist":
        return _MnistDataset(root, task.dataset.sample_count)
    if dataset_name == "california_housing":
        return _CaliforniaHousingDataset(root, task.dataset.sample_count, seed)
...
_CHECKPOINT_SUFFIX_RE = re.compile(r"-(\d+)\.pkl$")


def list_same_family_checkpoints(task: MLTaskSpec, n: int) -> tuple[str, ...]:
    """RQ3 same-family predicate: identical architecture+dataset+loss+grid;
    only trained weights differ. Convention: `{family}-{seed}.pkl` in `assets/`."""
    if task.checkpoint_path is None:
        raise ValueError(f"workload {task.name!r} has no checkpoint_path")
    template = Path(task.checkpoint_path)
    match = _CHECKPOINT_SUFFIX_RE.search(template.name)
    if not match:
        raise ValueError(
            f"checkpoint_path {task.checkpoint_path!r} does not match family pattern "
            "{family}-{seed}.pkl"
        )
    family = template.name[: match.start()]
    parent = template.parent
    asset_dir = resolve_asset(parent)
    matches = sorted(
        asset_dir.glob(f"{family}-*.pkl"),
        key=lambda path: int(_CHECKPOINT_SUFFIX_RE.search(path.name).group(1)),
    )
    if len(matches) < n:
        raise FileNotFoundError(
            f"need {n} same-family checkpoints for {family}, found {len(matches)} under {asset_dir}"
        )
    return tuple(str(parent / path.name) for path in matches[:n])
